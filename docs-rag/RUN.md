# basset-hound-docs-rag — RAG consumer (single mode)

Generated by `rag new-consumer` on 2026-07-06. Runs PRE-BUILT
rag-bootstrap images (RAG_IMAGE_TAG=0.3.0). No application source lives here.
...
## Boot
```sh
docker compose up -d          # web UI on http://127.0.0.1:10080
docker compose logs -f api
```

## Query
```sh
RAG_ENDPOINT_URL=http://127.0.0.1:10080 python3 client/ragq.py "your question"
curl -sf -X POST http://127.0.0.1:10080/api/v1/search \
...
```sh
bash ops/relocate_data.sh --to /mnt/quickiespace/basset-hound-docs-rag            # plan only
bash ops/relocate_data.sh --to /mnt/quickiespace/basset-hound-docs-rag --apply    # execute
```
