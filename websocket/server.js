...
class WebSocketServer {
  ...
  _setupStateRollbackListeners() {
    ...
    this.stateManager.registerRollbackListener('proxy', async (snapshot) => {
      if (!snapshot.stateData.config) return;
    ...
    this.stateManager.registerRollbackListener('tor_mode', async (snapshot) => {
      if (!snapshot.stateData.torMode) return;
    ...
    this.stateManager.registerRollbackListener('navigation', async (snapshot) => {
      if (!snapshot.stateData.currentUrl) return;
    ...
    this.commandHandlers.navigate = async (params) => {
        ...
        async (snapshot) => {
          ...
          if (snapshot.stateData.currentUrl) {
            ...
            try {
              ...
              this.logger.warn(`[Navigate] Rollback timeout for ${snapshot.stateData.currentUrl}`);
            }
          }
        }
      );
      ...
      if (result.success === false && result.rollbackAttempted !== true) {
        ...
        if (result.error?.includes('timeout')) {
          this.logger.warn(`[Navigate] Timeout waiting for completion of ${url}`);
          return {
            success: true,
            url,
            timeout: true,
