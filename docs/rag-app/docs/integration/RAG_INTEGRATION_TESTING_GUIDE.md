...
#    (place your .md/.pdf/.txt sample docs in ./data/docs first — the directory is
#    created on first compose up and mounted into the container at /data/docs)
./deploy.sh ingest /data/docs
...
Location: `data/docs/` (repo root — created on first `docker compose up`, mounted into the
container at `/data/docs`, and watched by the auto-ingest watcher).

> **Note**: No sample fixtures ship with the repo. Place your own domain documents in
> `data/docs/` before running the accuracy benchmark. The benchmark questions below assume
> aviation/ATC-domain content, e.g. an aviation safety manual (`aviation_safety_manual.md`) covering:
...
**Solution**:
```bash
# Ingest sample documents (placed in ./data/docs, mounted at /data/docs in the container)
./deploy.sh ingest /data/docs

# Or upload via API
curl -X POST http://localhost:8100/api/ingest/file \
  -F "file=@data/docs/aviation_safety_manual.md"
...
- [`RAG_PRODUCTION_API.md`](./RAG_PRODUCTION_API.md) - API reference
- `README.md` (repo root) - System overview
- [`docs/todo.md`](../todo.md) - Current status
